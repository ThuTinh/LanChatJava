            ...
            try {
                ...
                switch (msg.type) {
                    ...
                    case "signout": {
                        ...
                        for (int i = 0; i < groupChatFrames.length; i++) {
                            if (groupChatFrames[i].isEnabled()) {
                                for (int j = 0; j < groupChatFrames[i].onlineModel.size(); j++) {
                                    if (msg.content.equals(groupChatFrames[i].onlineModel.getElementAt(j))) {
                                        groupChatFrames[i].onlineModel.removeElementAt(j);
                                        break;
                                    }
                                }
                            }
...
//                                }
                    ...
                    case "GroupChatGetUserList": {
                        ...
                        groupChatFrames[groupId].onlineModel.removeAllElements();
                        for (int i = 0; i < mainFrame.model.size(); i++) {
                            groupChatFrames[groupId].onlineModel.addElement(mainFrame.model.getElementAt(i).toString());
                    ...
                    case "GroupLeave": {
                        if (msg.content.equals(mainFrame.username)) {
                            ...
                            if (msg.content.equals("fail")) {
                                JOptionPane.showMessageDialog(mainFrame, "Không thể rời khỏi nhóm");
                            }
                        }
                        if (!msg.content.equals(mainFrame.username) && !msg.content.equals("fail")) {
                            //this.send(new Message("GroupChatGetUserList", mainFrame.username, msg.recipient, "Server"));
                            int groupID = FindGroupFame(msg.recipient);
