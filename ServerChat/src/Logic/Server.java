...
public class Server implements  Runnable{
    ...
    public  void Announce(String type , String sender , String content) throws IOException
    {
        Message msg = new Message(type, sender, sender, "All");
        for(int i = 0; i<count;i++)
        {
            clients[i].send(msg);
        }
    ...
    private void GroupLeave(int portClient, Message msg) {
        try {
            db.DeleteGroupChatMember(msg.getRecipient(), msg.getSender());
            int groupId = FindGroupName(msg.getRecipient());
             ...
             if(!msg.getContent().equals("No"))
        {
            if(FindUServerThread(msg.getSender().toString()) !=null)
            {
               ...
               // FindUServerThread(msg.getRecipient()).send(new Message("UploadRes",msg.getSender(),msg.getContent(),msg.getRecipient()));
                FindUServerThread(msg.getRecipient()).send(new Message("UploadRes", Ip, msg.getContent(), msg.getRecipient()));
